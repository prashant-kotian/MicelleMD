...
(Direction (b) is now done -- see MicelleMD's `ROADMAP.md`, the real MARTINI 3 parameters and the CG production run. This section continues with direction (a).)

## Scaled-up run launched -- 2026-09-03, 100 molecules, targeting a real micelle-scale aggregation number

Direction (a) above, actually started. Working directory: `~/scratch_md/multi_sds_100` on the Ubuntu machine (separate from the completed 20-molecule run's `~/scratch_md/multi_sds`).

Same proven pipeline, scaled 5x: `gmx insert-molecules` placed 100 dodecyl sulfate anions randomly in an 8x8x8 nm box (no overlap failures, `-try 500` sufficient); `write_combined_topology(n_molecules=100)` -- the exact same reusable function from the 20-molecule run, no code changes needed -- generated the combined topology; `gmx solvate` added 15,391 SPC-geometry waters (50,373 atoms total, density 985.3 g/L, correctly close to real water); `gmx genion` neutralized with 100 Na+ (replacing the -100 net charge from 100 sulfate anions).

**Real memory check done before committing to a long run** -- prompted directly by a separate, unrelated finding the same day (SurfQM's collaborator-protocol DFT batch got OS-killed by a RAM ceiling on this same machine, see SurfQM's `ROADMAP.md`). Checked GROMACS specifically rather than assuming it would also be fine: energy minimization on the full 50,373-atom system, measured via `/usr/bin/time -v`, peaked at **83 MB resident memory** -- three orders of magnitude below this machine's 7.5 GB, confirming GROMACS's domain-decomposition/neighbor-list memory model scales nothing like DFT's basis-set-driven memory growth. No risk here, checked rather than assumed.

Minimization converged in 1685 steps, PE = -801,017 kJ/mol (proportionally more negative than the 20-molecule system's -316,068 kJ/mol, as expected for ~5x the molecules). Short 10 ps NVT stability check ran cleanly. Benchmarked at 2 thread counts before choosing: 4 threads gave 13.49 ns/day, 8 threads gave 13.76 ns/day -- essentially flat, unlike the earlier 20-molecule system where 8 threads was measurably *slower* than 4 (likely a real system-size-dependent parallel-efficiency effect, not a fluke -- worth remembering that a threading conclusion from one system size doesn't automatically transfer to another). Used 8 threads (full machine) for the production run since it's at least as fast and uses all available cores.

**Production run launched**: NVT, 2 fs timestep, V-rescale thermostat @ 300 K, PME electrostatics -- identical physics settings to the completed 20-molecule run, just `nsteps` retargeted for a fresh 20 ns (10,000,000 steps) rather than the earlier run's original 10 ns + 10 ns extension. Launched via `nohup ... < /dev/null &` plus `disown` (the same detachment pattern already documented as necessary for this machine -- a plain `nohup cmd &` over a non-interactive SSH command can hang the parent SSH session even though the child detaches correctly), independently verified running via a fresh SSH connection afterward (PID confirmed, TTY `?`, i.e. genuinely detached from any controlling terminal) rather than trusting the launch command's own exit. At 13.76 ns/day, 20 ns is expected to take roughly 1.5 days wall time. Disk footprint estimated by `grompp` at 491 MB -- comfortably inside the 140 GB free on this machine.

**Expected outcome, stated before the result is known**: with 5x the monomer pool of the completed run (100 vs. 20) and CG-route precedent already showing larger monomer pools reach substantially larger aggregates (150-molecule CG run: 22-molecule aggregate by 12.5 ns), this system has a real chance of reaching something close to SDS's actual literature aggregation number (~44.8-54.2, Bales et al. 1998) -- but this is a real simulation, not guaranteed, and will be reported honestly regardless of what it actually shows, same as every other run in this project.
